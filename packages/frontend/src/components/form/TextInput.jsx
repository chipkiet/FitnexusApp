import React from 'react';

export default function TextInput({ label, name, value, onChange, type = 'text', placeholder, required = false, showValidationIcon = false, isValid = false }) {
  return (
    <label className="block text-sm">
      {label && <span className="block mb-1 font-medium text-gray-700">{label}</span>}
      <div className="relative">
        <input
          className={`w-full px-3 py-2 transition-colors bg-white border rounded outline-none border-gray-200 focus:border-blue-500 focus:ring-2 focus:ring-blue-100 ${
            showValidationIcon && isValid ? 'pr-10' : ''
          }`}
          name={name}
          type={type}
          value={value}
          onChange={onChange}
          placeholder={placeholder}
          required={required}
          autoComplete="on"
        />
        {showValidationIcon && isValid && value && (
          <div className="absolute inset-y-0 right-0 flex items-center justify-center w-10">
            <svg className="w-5 h-5 text-green-500" fill="currentColor" viewBox="0 0 20 20">
              <path fillRule="evenodd" d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z" clipRule="evenodd" />
            </svg>
          </div>
        )}
      </div>
    </label>
  );
}

