      ...
      <header className="sticky top-0 z-20 border-b border-white/10 bg-black/30 backdrop-blur-xl">
        <div className="max-w-7xl mx-auto px-6 py-4 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <h1 className="text-xl font-bold text-white tracking-tight">FitNexus 3D Studio</h1>
          </div>

          <div className="flex items-center gap-3">
            <button onClick={() => navigate('/')} className="hidden md:flex items-center gap-2 px-3 py-2 rounded-lg bg-white/10 text-white hover:bg-white/20">Homepage</button>
            {user && (
            ...
            <button onClick={logout} className="px-3 py-2 rounded-lg bg-red-500/80 hover:bg-red-500 text-white">Logout</button>
          </div>
        </div>
      </header>
        ...
        <aside className="space-y-4">
          <div className="p-4 rounded-2xl ring-1 ring-white/10 bg-white/5 backdrop-blur-xl">
            ...
            <div className="flex flex-wrap gap-2">
              {BODY_PARTS.map((b) => (
                <button key={b.key} onClick={() => handleBodyPartClick(b.key)} className={`px-3 py-1.5 rounded-full text-sm border ${selectedPart===b.key?'bg-fuchsia-600 border-fuchsia-500 text-white':'border-white/20 text-white/80 hover:bg-white/10'}`}>{b.label}</button>
              ))}
          ...
          <div className="p-1 rounded-xl bg-white/5 ring-1 ring-white/10 backdrop-blur-xl">
            ...
            {activeTab === 'overview' && (
              <div className="p-4 text-sm text-white/80 space-y-2">
                <p>Welcome to FitNexus 3D Studio. Pick a muscle group and explore curated exercises. Personalize your plan with onboarding, then view your plan preview.</p>
                <ul className="list-disc pl-5 space-y-1">
                  <li>Click model parts or use Quick Select.</li>
                  <li>Filter exercises using the search below.</li>
                  <li>Use the actions below to continue.</li>
                </ul>
              </div>
            )}
